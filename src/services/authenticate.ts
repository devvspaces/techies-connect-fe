...
export const authenticate = (data: LoginResponse) => {
  Cookies.set(PAYLOAD_KEY, JSON.stringify(processJWTPayload(data.tokens)));
...
export const getAccessToken = () => {
  ...
  // TODO: Add this process for updating cookie in the getServerSide Props too
  return {
    ...jwt_payload_decoded,
    updatePayload: (payload: JwtPayload) => {
      Cookies.set(PAYLOAD_KEY, JSON.stringify(processJWTPayload(payload)));
    },
  };
};
